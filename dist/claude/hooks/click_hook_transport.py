...
def _plugin_manifest_relative(root: Path) -> str:
    """Prefer the host manifest that is actually installed alongside the hooks."""
    for relative in PLUGIN_MANIFESTS:
        if (root / relative).is_file():
            return relative
    return PLUGIN_MANIFESTS[0]
...
def _plugin_identity() -> str | None:
    ...
    try:
        ...
        for relative in (_plugin_manifest_relative(root), "hooks/hooks.json"):
            path = root / relative
            with path.open("rb") as handle:
                payload = handle.read(remaining_bytes + 1)
            remaining_bytes -= len(payload)

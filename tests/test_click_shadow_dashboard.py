...
class ClickShadowDashboardTests(ClickGateTestCase):
    ...
    def test_dashboard_javascript_parses_and_keeps_contract_prose_out_of_share_report(self) -> None:
        ...
        self.assertNotIn("reuse_origin", share)
        self.assertIn("task_efficiency", share)
        ...
        self.assertIn('data-i18n="순작업시간"', html)
        self.assertIn('data-i18n="토큰 절감률"', html)
        self.assertIn('data-i18n="전체 작업 효과"', html)
        self.assertIn("과거 실행 기록 기반 추정", html)
        self.assertIn("동일 묶음·순차 실행 기준", html)
        self.assertIn("관리비용 별도", html)
        self.assertIn("이번 요청의 실측 결과가 아닙니다", html)
        self.assertIn("@media(max-width:760px)", CLICK_SHADOW_DASHBOARD.CSS)
        self.assertIn(":focus-visible", CLICK_SHADOW_DASHBOARD.CSS)
        self.assertIn(
            "@media(prefers-reduced-motion:reduce)",
            CLICK_SHADOW_DASHBOARD.CSS,
        )
    ...
    def test_server_is_loopback_read_only_authenticated_and_content_free(self) -> None:
        ...
        with urllib.request.urlopen(base + "/", timeout=2) as response:
            ...
            self.assertIn("작업 범위와 승인 근거", html)

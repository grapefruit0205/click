...
def main():
    ...
    bytecode_prefix = out / 'empty-bytecode-cache'
    ...
    manifest = {'kind': 'click-phase6-paired-source-guarded-fixture', 'started_at': stamp(),
                'environment': {'python': sys.version, 'executable': python, 'platform': platform.platform(),
                                'pythonhashseed': '0', 'dont_write_bytecode': True,
                                'pythonpycacheprefix': str(bytecode_prefix),
                                'bytecode_cache_condition': 'empty isolated prefix for both versions; existing source and stdlib bytecode caches bypassed',
                                'pythonpath_and_pythonhome': 'removed from benchmark children',
                                'os_cache': 'not-flushed', 'parallel_benchmark_processes': 1},
    ...
    for item in schedule:
        ...
        run['bytecode_prefix_absent_before'] = not bytecode_prefix.exists()
        try:
            ...
            run.update(exit_code=code, timed_out=timed_out, subprocess_overall_ms=elapsed,
                       bytecode_prefix_absent_after=not bytecode_prefix.exists())
            if not run['bytecode_prefix_absent_before'] or not run['bytecode_prefix_absent_after']:
                raise ValueError('Bytecode prefix unexpectedly populated; cache conditions invalid.')

...
> **同一个测试不跑第二次，它的输出不读第二次，也绝不凭猜测跳过。**

只改了登录逻辑，编码代理却又跑了一遍完整测试，又把全部输出读了一遍。
再做一个小修改，同样的等待和同样的上下文消耗再来一次。

**Click 是管理验证与复用的工作流护栏（workflow guardrail），保持你选择的
模型和推理设置不变。** 实现方式仍由模型决定。Click 为每次检查记录精确凭据，
在执行时判定哪些检查必须重跑，并用一行凭据代替被跳过执行的完整输出。只有当
同一状态的 exact 凭据、已签名的输入观测或已提交的所有者策略证明结果仍然有效时
才会跳过；有任何不确定就执行。

这就是**增量验证（incremental verification）**。支撑判断的
**revision-aware evidence** 记录“什么通过了，现在是否仍然有效”。每个判定都带有
原因代码，仪表板会显示复用在哪里丢失。节省针对重复的执行与重复的阅读；
Click 不通过更换更弱的模型或降低推理设置来节省成本。
...
- **少花上下文：** 被复用的检查只返回一行凭据；Evidence 模式下受支持的 Python 运行器默认报告有界的失败摘要而不是原始输出。
- **绝不错误跳过：** 复用需要 exact 凭据、已签名的输入观测或已提交的所有者策略之一；不确定时总是执行。
- **解释每次决定：** 看清哪些执行了、哪些被复用、依据是什么，以及保留历史中的重跑原因分布。
- **让结果延续到后续任务：** 把先前成功作为候选，重新核对当前条件。
- **查看实际结果：** 在本地仪表板区分验证结果、估算的避免执行时间，以及宿主未再次读取的输出。

Click 更适合**测试耗时长、频繁修改并验证、测试可合理分组的项目**。
如果整个套件只需两秒，设置与判断成本可能更高。目标是减少完成相同工作所需
的时间与麻烦；真实项目节省多少分钟和 Token，仍需单独测量。
已有一个实测数据点：[八个 Opus 5 配对会话](docs/history/agent-ab-2026-09-12/README.md)在一个 40 秒的 fixture 上完成同一修复任务，
有 Click 用时 123 秒、无 Click 用时 381 秒（−68%，测试执行 −50%），成本 +6~11%。这是 fixture 记录，不是真实项目结果。
...
当前版本：**v1.3.0**。更新后请重启，并使用新任务。

v1.0.0 发布了 `main` 上原为 v0.97 候选的内容：带签名凭据的自动输入观测、条件 JS 复用、捕获丢失后的恢复、Evidence 模式下的自动分片、分片并发执行，以及跨新宿主会话保留的凭据。v1.0.1 修复了 Claude Code 将其他已安装插件的命令目录加入搜索路径时凭据失效的问题。v1.1.0 接受简写形式 `click-gate verify -- <检查命令>` 并引导代理使用它；在另外两个实测会话中，开启 Click 的费用降到了关闭时的水平。v1.1.1 将结果行、复用提示、`click-gate status` 和仪表板的默认语言改为英语，韩语和简体中文由区域设置选择。v1.2.0 通过 Git Bash 支持 Windows 上的 Claude Code：POSIX Hook 启动器、改写命令的 Git Bash 渲染，以及环境指纹中 Windows 搜索路径条目的统一拼写，使凭据得以复用。v1.3.0 将条件 JS 复用带到 Windows：ETW 后端像 Linux 上的 strace 一样投影每个被观测 Node 进程的输入，检查器收集器以 Windows 专用配置运行，原生随机状态读取器用 MSVC 和 Node 头文件构建，由 `node` 运行的框架入口点被识别为该框架的检查。缺少 Python 3.10+ 时，Click 会提示安装而不是让每个事件失败。两个市场均固定为 `v1.3.0`。详见[版本说明](RELEASE_NOTES.md)。
...
在 Evidence 模式下，验证请求无需指明 Click。Click 能识别的普通检查命令
（`python3 -m pytest -q 2>&1 | tail -5`、`npm test`、`npm run build`、`go test ./...`）
会由 Hook 转为 `click-gate verify -- <argv>`，因此不遵循 Evidence 指令的模型也能获得
凭据与复用。命令自带的 `2>&1` 和 `| tail` 等输出过滤照常生效。写入仓库文件的检查
（构建、快照更新）保留自身退出码，记录为工作区变更，而不是失败或可复用的结果。需要
shell 的命令（展开、`&&`、重定向、写文件或执行命令的过滤）照常执行。在 Claude Code
中，转换后的命令带有与 `click-gate verify` 相同的 `allow`，因此跳过权限提示；设置
`CLICK_AUTO_ROUTE=0` 可关闭。
...
| **Guarded — 按需启用** | 带强制力的 plan 模式：提出可读的合约，等待用户在后续轮次明确批准，然后由 Hook 拒绝批准边界之外的变更，把承诺的检查绑定到凭据，并通过轮次分离与 digest 防止自我批准。只有在本来会出问题的那一天你才会感觉到它。 |
...
| 条件 Python 观测 | 仅剩已跟踪子进程、线程或动态自省的原生观测会把输入快照保留为条件凭据。复用仅限于观测输入未变的情况，并标注完整性未证明。 |
| 输入身份与环境 | 观测输入按内容、类型和成员关系识别，而非时间戳或 inode。凭据只指纹解释器、工具链、区域设置、路径和代理变量以及所有者的 `.click/environment.json`；其他会话变量视为运行时假设。 |
...
**右上角的语言选择器**提供 **한국어 · English · 简体中文**。浏览器语言为韩语或简体中文时默认使用该语言，其余情况默认英语；本地存储可用时，浏览器会记住同一来源的显式选择。报告跟随所选语言，用户编写的任务名称和检查名称则保留原文。
...
`click-gate status` 打印简短的只读摘要：运行与复用数量及估算节省时间、模式与修订号、下一步操作，语言由 `CLICK_LANGUAGE` 或 POSIX 区域设置决定（区域设置不是韩语或简体中文时为英语）。运行器的 `[Click 结果]` 行和复用提示使用同一语言。`click-gate status --json` 返回完整报告，包括已执行、已复用、未执行或尚未请求的检查、变更后失效的状态、逐项原因代码和失败诊断。两种形式都只报告已登记的验证依据，不代表整个任务的正确性，也不授予复用。
...
在 Claude Code 中，用 `claude plugin list` 查看已安装插件，用 `/hooks` 查看 `[plugin:click]` 的 Hook 定义；源码构建可用 `claude plugin validate ./dist/claude --strict` 检查。Hook 命令为 `sh "${CLAUDE_PLUGIN_ROOT}/hooks/claude_hook.sh"`，Claude Code 在 Linux 和 macOS 上通过 `sh -c` 运行它，在 Windows 上通过 Git Bash 运行；启动器在 Linux 和 macOS 上运行 `python3`，在 Windows 上依次尝试 `py -3`、`python`、`python3`，并跳过只会提示安装 Python 的 Microsoft Store 别名和只会提示安装命令行工具的 macOS 存根。Click 需要 Python 3.10 或更高版本：没有可用解释器时，提示 Hook 会显示安装说明并告知模型在新会话之前直接运行检查，工具调用不会被阻塞。Windows 需要安装 Git for Windows：没有它，Claude Code 只提供 PowerShell 工具，而 Click 尚未改写该工具。Hook 的输出和错误会以 `click hook error` 行出现在对话记录中。

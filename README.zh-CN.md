...
当前版本：**v1.0.0**。更新后请重启，并使用新任务。

v1.0.0 发布了 `main` 上原为 v0.97 候选的内容：带签名凭据的自动输入观测、条件 JS 复用、捕获丢失后的恢复、Evidence 模式下的自动分片、分片并发执行，以及跨新宿主会话保留的凭据。两个市场均固定为 `v1.0.0`，从 v0.96.1 更新即可安装。详见[版本说明](RELEASE_NOTES.md)。

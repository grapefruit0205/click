...
当前版本：**v1.1.0**。更新后请重启，并使用新任务。

v1.0.0 发布了 `main` 上原为 v0.97 候选的内容：带签名凭据的自动输入观测、条件 JS 复用、捕获丢失后的恢复、Evidence 模式下的自动分片、分片并发执行，以及跨新宿主会话保留的凭据。v1.0.1 修复了 Claude Code 将其他已安装插件的命令目录加入搜索路径时凭据失效的问题。v1.1.0 接受简写形式 `click-gate verify -- <检查命令>` 并引导代理使用它；在另外两个实测会话中，开启 Click 的费用降到了关闭时的水平。两个市场均固定为 `v1.1.0`。详见[版本说明](RELEASE_NOTES.md)。

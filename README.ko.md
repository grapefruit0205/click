...
현재 릴리스는 **v1.0.1**입니다. 업데이트 후에도 재시작하고 새 작업을 사용합니다.

v1.0.0은 `main`이 v0.97 후보로 갖고 있던 내용을 공개했습니다. 서명된 영수증을 남기는 자동 입력 관찰, 조건부 JS 재사용, 캡처 손실 후 복구, Evidence 모드의 자동 샤딩, 샤드 동시 실행, 새 호스트 세션에서도 이어지는 영수증이 포함됩니다. v1.0.1은 Claude Code가 다른 설치 플러그인의 명령 디렉터리를 검색 경로에 붙여도 영수증이 유효하도록 고칩니다. 두 마켓플레이스가 모두 `v1.0.1`을 고정합니다. [릴리스 노트](RELEASE_NOTES.md)를 참고하세요.

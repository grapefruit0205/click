...
기본 JavaScript 관찰은 별도 JSON 없이 **조건부 재사용** 근거도 만들 수 있습니다. 정상적으로 요청된 두 번의 실행에서 관찰 입력을 학습·대조하고, 다음 요청마다 다시 확인합니다. 대시보드와 리포트는 입력 완전성 미보증을 표시합니다. 확인된 동적 입력이나 수집 공백이 있으면 해당 자식을 실행하며, 일부 Vitest/Jest·worker 실행은 아직 대상이 아닙니다. [조건부 범위와 한계](docs/architecture/node-runtime-observation.md)를 참고하세요.

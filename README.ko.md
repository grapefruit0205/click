...
**Click은 AI 코딩 에이전트가 아직 유효한 검증을 반복하는 비용을 줄입니다.**
어떤 조건에서 성공했는지 기억하고, 수정 후에도 그 조건이 유지되는지 확인한 뒤
새 근거가 필요한 검사를 실행합니다. 이를 **점진적 검증(incremental verification)**,
그 판단에 쓰는 변경 이력과 검증 기록을 **revision-aware evidence**라고 부릅니다.

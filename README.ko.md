...
> *Verify what changed. Reuse what didn't.*

Click은 AI 코딩 에이전트를 위한 **검증 재사용 레이어(Verification Reuse Layer)**이자 **점진적 검증(incremental verification)** 도구입니다. 이전에 성공한 결과만 저장하는 데서 멈추지 않고, 어떤 명령·코드·환경·정책에서 성공했는지 **revision-aware evidence**로 함께 기록합니다. 현재 조건과 근거가 일치하면 그 결과를 재사용하고, 관련 입력이나 조건이 달라졌다면 실제 검증을 다시 수행합니다.

목표는 필요한 검증을 빠짐없이 수행하면서 아직 유효한 검증의 반복을 줄여, 합의한 같은 작업을 더 적은 시간·토큰·사용자 개입으로 완성하는 것입니다. 검사를 재사용한 횟수는 그 과정의 일부이며, 그것만으로 전체 작업이 빨라졌다고 판단하지 않습니다.

현재 소스에는 parent 대비 비용 판정, 중복 초기 검증 축소, Vitest/Jest의
안정적인 파일 묶음, 소유자가 선언하는 파일 입력별 재사용 정책이 포함됩니다.
[검증 비용과 입력 정책](docs/architecture/verification-economics.md)에서 설정과
세션 비교 방법을 확인할 수 있습니다. 실제 프로젝트에서 수십 분 절감은 아직 검증하지 않았습니다.

- **근거가 있는 재사용:** 실행 조건과 재사용 규칙이 여전히 맞는 결과를 유지합니다.
- **자동 샤딩:** 지원하는 테스트 묶음의 분할을 제안하고 관리합니다. 분할할 수 없거나 이득이 없으면 원래 전체 명령을 유지합니다.
- **검증 상태와 실패 진단:** 실제 실행·재사용·실패·남은 검사를 구분하고, 필요하면 실패를 요약합니다.
- **로컬 대시보드:** 한국어·영어·중국어 간체로 결과, 재사용 근거, 측정값, 공유 리포트를 확인합니다.

Click은 코드가 올바르거나 선택한 테스트가 충분하다고 증명하지 않습니다.

## 왜 Click이 필요한가?

AI 코딩 에이전트는 한 작업 안에서도 코드 수정과 검증을 여러 차례 반복합니다.

```text
코드 탐색 → 수정 → 검증 → 다시 수정 → 검증 → 실패 수정 → 다시 검증
```

프로젝트가 커질수록 작은 변경 뒤에 모든 검증을 처음부터 반복하는 비용도 커집니다. Click은 이 흐름에 **검증 상태(Verification State)**를 추가해 다음 질문을 매번 다시 판단합니다.
...
Click은 테스트를 임의로 줄이지 않습니다. 다시 실행할 필요가 없는 검증과 새로 확인해야 하는 검증을 근거에 따라 구분합니다.

## 무엇을 추적하나요?

검증 경로에 따라 다음 문맥을 함께 확인합니다.

- 실행한 정확한 검증 명령
- 저장소와 Git revision, 최종 작업 공간 상태
- 환경 조건과 사용한 실행 파일
- 명시되거나 관찰된 의존성
- 적용된 변경·재사용 정책
- 이전 검증 영수증과 현재 유효성

> **예전에 이 테스트가 통과했다**는 사실과 **그 결과를 지금도 신뢰할 수 있다**는 판단은 다릅니다.

예를 들어 Frontend·Backend·Database 검증이 모두 성공한 뒤 Backend 코드만 바뀌었다고 가정해 보겠습니다. 각 묶음에 대한 충분한 의존성 근거와 재사용 조건이 있다면 Backend 검증은 다시 실행하고 나머지는 재사용할 수 있습니다. 그 근거가 없거나 불완전하면 Click은 영향을 추측하지 않고 해당 검증을 다시 실행합니다.

일반적인 테스트 캐시가 “같은 입력으로 실행한 적이 있는가?”를 묻는다면, Click은 “이전에 확보한 결과를 현재도 신뢰할 근거가 충분한가?”를 묻습니다. 이를 위해 검증 근거, 무효화, 후속 작업 재판정, 변경 정책, 부분 검증, 의존성 기반 재사용, 실행 권한을 서로 구분합니다.
...
현재 릴리스는 **v0.96.1**이며, v0.96.0에 Claude Code 플러그인을 더한 버전입니다. 업데이트 후에도 재시작하고 새 작업을 사용합니다. v0.96.0은 손해가 되는 분할을 제외하고, Evidence bootstrap의 자식 중복 실행을 줄이며, 대규모 Vitest/Jest 묶음과 소유자가 선언하는 파일 입력 정책 v2를 지원합니다. 영향받거나 불확실한 자식은 실행하고 분할이 불완전하면 parent로 돌아갑니다. 자동 샤딩 `init/status/refresh`와 권한 있는 샤드 재사용은 필수 회귀 기준입니다. 자세한 내용은 [릴리스 노트](RELEASE_NOTES.md)와 [검증 비용 및 입력 정책](docs/architecture/verification-economics.md)에 있습니다.
...
패키지 설치
  ...
  → Sharding 구성
  → Baseline 검증
  → 조건을 충족한 권위 있는 재사용 준비
```

프로젝트에 따라 Evidence 기록과 정확한 영수증 재사용만 사용할 수도 있습니다. 모든 고급 기능을 처음부터 활성화할 필요는 없습니다.

## 평소 작업부터 시작하기

Codex에 평소처럼 요청하면 됩니다.

```text
인증 파서를 리팩터링하고 외부 동작은 유지해줘.
저장소의 관련 테스트를 Click Evidence로 실행하고 click-gate status로 결과를 보여줘.
```

이 문서의 `click-gate`는 Codex 작업 안에서 에이전트가 실행하는 Click 제어 명령입니다. 위의 설치 명령은 터미널에서 실행합니다.
...
| 권위 있는 입력 관찰 | 지원되는 Guarded 실행에서 명시적으로 활성화한 관찰이 완전한 서명된 입력 스냅샷을 남겨야 하며, 재사용 조건도 다시 확인합니다. |
...
**네. 기본값이 Off입니다.** Observer를 꺼도 Evidence 기록, 일반 검증, 대시보드, 조건을 충족한 정확한 영수증·안전 변경 정책 재사용이 동작합니다.
...
- `click-gate observer authoritative`: 별도로 승인된 Guarded 계약, 지원되는 CPython **3.12.3** 직접 unittest 명령, 플랫폼별 사전 준비가 필요합니다. 켜는 것만으로는 부족하며 완전한 서명 관찰이 있어야 재사용할 수 있습니다.
...
Click은 전체 검증 비용이 크고, 에이전트가 수정과 검증을 자주 반복하며, 검증을 의미 있는 묶음으로 나눌 수 있는 프로젝트에서 효과가 큽니다. 특정 영역의 변경이 일부 검증에만 영향을 주고, 후속 작업에서도 이전 결과를 재평가할 수 있는 환경에 특히 잘 맞습니다.

전체 테스트가 2~3초 안에 끝나는 작은 프로젝트에서는 상태를 판단하고 기록하는 비용이 그냥 다시 실행하는 비용보다 클 수 있습니다. 이때는 샤딩이나 권위 관찰을 무리하게 켜기보다 기본 Evidence 기록으로 시작해 실제 전체 작업시간을 비교하세요.

## Click이 하지 않는 것

- 프로그램의 논리적 정확성을 수학적으로 증명하지 않습니다.
- 테스트의 충분성, 커버리지 품질, 결함 있는 테스트의 정확성을 보증하지 않습니다.
- 모든 외부 의존성과 동적 상태를 자동으로 발견하지 않습니다.
- 모든 프로젝트의 속도 향상을 보장하지 않습니다.
- 모델의 추론 능력이나 컨텍스트를 줄여 비용을 아끼지 않습니다.

Click은 모델의 판단 능력을 유지하면서, 명확한 근거가 있는 기계적 재검증의 반복을 줄입니다.
...
1. **검증에는 기억이 있어야 합니다.** 새 턴이 시작됐다는 이유만으로 유효한 성공 근거를 버리지 않습니다.
2. **재사용에는 근거가 필요합니다.** 과거의 PASS만으로 현재 재사용을 허용하지 않습니다.
3. **불확실하면 다시 검증합니다.** 유효 조건을 확인할 수 없으면 실제 검증으로 돌아갑니다.

> 워크플로 비용을 줄이기 위해 모델을 약하게 만들지 않습니다. 다시 할 필요가 없는 작업을 줄입니다.

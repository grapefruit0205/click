...
> **같은 테스트를 두 번 돌리지 않고, 그 출력을 두 번 읽지 않습니다. 그리고 추측으로 건너뛰지 않습니다.**

로그인 코드 하나를 고쳤는데 에이전트가 전체 테스트를 다시 실행하고, 그 출력을
전부 다시 읽습니다. 작은 수정을 한 번 더 하면 같은 대기와 같은 컨텍스트 소비가
반복됩니다.

**Click은 모델의 추론 설정을 바꾸지 않고, 개발 과정의 검증과 재사용을 관리하는
워크플로 가드레일(workflow guardrail)입니다.** 구현 방법은 여전히 모델이 정합니다.
Click은 검사마다 정확한 영수증을 남기고, 실행 시점에 어떤 검사를 다시 돌려야
하는지 판정하며, 건너뛴 실행은 전체 출력 대신 한 줄로 대체합니다. 같은 상태의
exact 영수증, 서명된 입력 관찰, 커밋된 소유자 정책 중 하나가 아직 유효함을
증명할 때만 건너뛰고, 애매하면 실행합니다.

이 방식을 **점진적 검증(incremental verification)**이라고 부릅니다.
판단 근거인 **revision-aware evidence**는 “무엇이 통과했고, 지금도 유효한가”를
확인할 수 있는 검증 기록입니다. 모든 판정에 이유 코드가 붙고, 대시보드는 재사용이
어디서 깨졌는지 보여줍니다. 모델을 낮은 등급으로 바꾸거나 추론량을 줄여
비용을 아끼는 방식이 아닙니다.
...
- **컨텍스트를 덜 씁니다.** 재사용된 검사는 영수증 한 줄만 돌려주고, Evidence 모드의 지원 Python 러너는 기본적으로 raw 출력 대신 실패 요약만 보고합니다.
- **틀리게 건너뛰지 않습니다.** 재사용에는 exact 영수증, 서명된 입력 관찰, 커밋된 소유자 정책 중 하나가 필요하고, 애매하면 항상 실행합니다.
- **왜 재사용했는지 보여줍니다.** 실행·재사용 여부와 판단 근거, 보관 이력의 재실행 사유 분포까지 확인할 수 있습니다.
- **다음 작업에도 성공 기록을 활용합니다.** 이전 결과를 후보로 가져와 현재 조건을 다시 확인합니다.
- **효과를 확인할 수 있습니다.** 대시보드에서 검증 결과, 추정 절약 실행시간, 호스트가 다시 읽지 않은 출력을 구분해 봅니다.
...
실측 자료는 하나 있습니다. [Opus 5 세션 8개 짝 비교](docs/history/agent-ab-2026-09-12/README.md)에서
40초짜리 fixture의 같은 버그 수정 작업이 Click 없이 381초, Click으로 123초에 끝났고(−68%,
테스트 실행 −50%), 비용은 +6~11%였습니다. fixture 기록이며 실제 프로젝트 결과가 아닙니다.
...
현재 릴리스는 **v1.3.0**입니다. 업데이트 후에도 재시작하고 새 작업을 사용합니다.

v1.0.0은 `main`이 v0.97 후보로 갖고 있던 내용을 공개했습니다. 서명된 영수증을 남기는 자동 입력 관찰, 조건부 JS 재사용, 캡처 손실 후 복구, Evidence 모드의 자동 샤딩, 샤드 동시 실행, 새 호스트 세션에서도 이어지는 영수증이 포함됩니다. v1.0.1은 Claude Code가 다른 설치 플러그인의 명령 디렉터리를 검색 경로에 붙여도 영수증이 유효하도록 고칩니다. v1.1.0은 `click-gate verify -- <검사 명령>` 축약형을 받아들이고 에이전트에게 그 형태를 안내하며, 추가 측정 세션 2개에서 Click 켠 쪽 비용이 꺼진 쪽 수준으로 내려왔습니다. v1.1.1은 결과 줄·재사용 안내·`click-gate status`·대시보드의 기본 언어를 영어로 바꾸고, 한국어와 간체 중국어는 로케일로 선택합니다. v1.2.0은 Git Bash를 통한 Claude Code Windows 지원을 추가합니다. POSIX Hook 실행기, 재작성 명령의 Git Bash 렌더링, 환경 지문의 Windows 검색 경로 철자 통일로 영수증이 재사용됩니다. v1.3.0은 조건부 JS 재사용을 Windows로 확장합니다. ETW 백엔드가 Linux의 strace처럼 관찰된 Node 프로세스의 입력을 투영하고, 인스펙터 수집기가 Windows 전용 프로파일로 동작하며, 네이티브 난수 상태 리더가 MSVC와 Node 헤더로 빌드되고, `node`로 실행한 프레임워크 엔트리포인트를 그 프레임워크의 검사로 인식합니다. Python 3.10+가 없으면 매 이벤트 실패 대신 설치를 안내합니다. 두 마켓플레이스가 모두 `v1.3.0`을 고정합니다. [릴리스 노트](RELEASE_NOTES.md)를 참고하세요.
...
Evidence 모드에서는 검증 요청에 Click을 명시하지 않아도 됩니다. Click이 인식하는
평범한 검사 명령(`python3 -m pytest -q 2>&1 | tail -5`, `npm test`, `npm run build`,
`go test ./...`)을 Hook이 `click-gate verify -- <argv>`로 보내므로, Evidence 지시를
따르지 않는 모델도 영수증과 재사용을 얻습니다. 명령에 붙은 `2>&1`과 `| tail` 같은
출력 필터는 그대로 적용됩니다. 저장소 파일을 쓰는 검사(빌드, 스냅샷 갱신)는 자기 종료
코드를 유지하고, 실패나 재사용 가능한 결과가 아니라 작업 공간 변경으로 기록됩니다. 셸이
필요한 명령(확장, `&&`, 리다이렉션, 파일을 쓰거나 명령을 실행하는 필터)은 그대로
실행됩니다. Claude Code에서는 `click-gate verify`와 같은 `allow`가 붙어 권한 확인
프롬프트를 건너뜁니다. `CLICK_AUTO_ROUTE=0`으로 끌 수 있습니다.
...
| **Guarded — 선택** | 강제력이 붙은 plan 모드입니다. 읽기 쉬운 계약을 제시하고 이후 사용자 턴에서 명시적으로 승인받은 뒤, 승인된 경계 밖 mutation은 Hook이 거부하고, 약속한 검사는 영수증으로 묶이며, 턴 분리와 digest로 자기 승인을 막습니다. 무언가 잘못될 뻔한 날에만 체감되는 안전장치입니다. |
...
| 조건부 Python 관찰 | 자식 프로세스 추적·스레드·동적 introspection만 남은 네이티브 관찰은 입력 스냅샷을 조건부 근거로 유지합니다. 재사용은 관찰된 입력이 그대로일 때로 제한되며 완전성 미입증을 표시합니다. |
| 입력 identity와 환경 | 관찰된 입력은 timestamp·inode가 아니라 내용·종류·멤버십으로 식별합니다. 영수증은 인터프리터·툴체인·로케일·경로·프록시 변수와 소유자의 `.click/environment.json`만 fingerprint하고, 그 밖의 세션 변수는 런타임 가정으로 둡니다. |
...
**오른쪽 상단 언어 선택기**에서 **한국어 · English · 简体中文**을 선택합니다. 브라우저 언어가 한국어·간체 중국어면 그 언어를, 그 외에는 영어를 기본으로 보여주며, 로컬 저장소를 사용할 수 있으면 브라우저가 같은 origin의 명시적 선택을 기억합니다. 리포트도 선택한 언어를 따르고, 사용자가 작성한 작업명과 검사명은 원문을 유지합니다.
...
`click-gate status`는 읽기 전용 요약을 짧게 출력합니다. 실행·재사용 수와 추정 절약 시간, 모드와 변경 번호, 다음 행동을 `CLICK_LANGUAGE` 또는 POSIX 로케일이 정한 언어(한국어·간체 중국어 로케일이 아니면 영어)로 보여줍니다. 러너의 `[Click 결과]` 줄과 재사용 안내도 같은 언어를 따릅니다. `click-gate status --json`은 실제 실행·재사용·미실행·미요청 검사와 코드 변경으로 무효화된 검사, 검사별 이유 코드, 실패 진단을 담은 전체 보고서를 반환합니다. 두 형식 모두 등록된 검증 근거의 상태를 보여줄 뿐 전체 작업의 정확성을 보장하거나 재사용을 승인하지 않습니다.

...
Click은 코딩 에이전트를 위한 **증분 검증(incremental verification)** 도구입니다. 어떤 검사를 실행했고 무엇이 바뀌었는지 기록하며, **revision-aware evidence**로 이전 결과가 현재 코드에도 유효한지 판단합니다. 기본 Evidence 모드는 호스트의 기존 권한으로 동작합니다. 승인 범위를 명확히 정해야 하는 작업에는 Guarded 모드를 선택할 수 있습니다.

목표는 합의한 같은 작업을 더 적은 시간·토큰·사용자 개입으로 완성하는 것입니다. 검사를 재사용한 횟수는 그 과정의 일부이며, 그것만으로 전체 작업이 빨라졌다고 판단하지 않습니다.
...
현재 릴리스는 **v0.92.0**입니다. 업데이트 명령은 다음과 같습니다.
...
업데이트 후에도 재시작하고 새 작업을 사용합니다. v0.92.0은 Hook 초기 로딩과 반복 실행 조건 확인 비용을 줄이고, 검증·대시보드 모듈을 분리하며, Linux·macOS·Windows에서 전체 CI 테스트 목록을 나누어 실행합니다. 변경 사항과 측정 범위는 [릴리스 노트](RELEASE_NOTES.md)와 [런타임 최적화 기록](docs/runtime-optimization.md)에 있습니다.
...
Click은 작업 흐름의 가드레일이며 운영체제 샌드박스가 아닙니다. 숨겨진 추론, 의미적 정확성, 테스트의 충분성, 일치하는 Hook 밖의 외부 활동을 증명하지 못합니다. 독립 관찰이 없는 수동·호스팅 근거는 보고자의 진술로 남습니다. 코드 리뷰, CI, 브랜치 보호, 배포 통제를 함께 사용하세요.
